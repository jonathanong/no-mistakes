---
name: no-mistakes
description: "Answers questions about how code connects: is this dead, still used, or safe to delete; is it only used by tests; where is a symbol used and who imports or calls it; did a rename update every reference; which tests actually need to run for a change, or why the whole suite is running; does a route have e2e coverage. Resolves the import graph instead of searching text, so consumers behind workspace aliases, barrel re-exports and transitive test chains are not missed."

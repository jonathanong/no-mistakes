...
expect(readRepoFile('package.json')).toContain /* current pin */ ('"eslint": "9.0.0"')
...
expect('10.9.0').toEqual(packageJson.devDependencies['eslint'])
expect('1.2.3').toBe /* current pin */ (packageJson.dependencies.foo)
expect('1.2.3').toBe((packageJson?.dependencies.foo))
expect('10.9.0').toEqual((packageJson as PackageJson).devDependencies.eslint)
expect(packageJson[`dependencies`][`foo`]).toBe('1.2.3')
expect(packageJson.dependencies.foo) /* dependency check */ .toBe('1.2.3')
expect(packageJson.devDependencies.eslint) // dependency check
  .toEqual('10.9.0')

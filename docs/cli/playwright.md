...
`--project`, `--app`, `--json`, `--assert-conditional-tests`,
...
frontend app. `--app <NAME>` is the `.no-mistakes.yml` `projects:` key of the
frontend app to analyze and matches the Node `app` option. When the repository
configures more than one `type: nextjs` project, set

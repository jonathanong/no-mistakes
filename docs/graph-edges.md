...
| `dotnet-project` | `DotnetProjectDependency` | `dotnet` | C# project source file -> files in a referenced `.csproj`; project -> owned `packages.lock.json`; every configured project -> nearest ancestor `Directory.Packages.props`, including projects affected only by `GlobalPackageReference`; a nested central manifest -> an explicitly imported ancestor central manifest | [`dotnet-test-plan`](../test-cases/codebase-analysis/dotnet-test-plan), semantic dependency-plan fixtures |

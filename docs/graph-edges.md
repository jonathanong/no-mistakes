...
| `dotnet-project` | `DotnetProjectDependency` | `dotnet` | C# project source file -> files in a referenced `.csproj`; project -> owned `packages.lock.json`; project -> nearest ancestor `Directory.Packages.props` when it declares package references | [`dotnet-test-plan`](../test-cases/codebase-analysis/dotnet-test-plan), semantic dependency-plan fixtures |

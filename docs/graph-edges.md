...
| `dotnet-project` | `DotnetProjectDependency` | `dotnet` | C# project source file -> files in a referenced `.csproj`; project -> owned NuGet `packages.lock.json` files, including TFM/RID variants; every configured project -> nearest ancestor `Directory.Packages.props`, including projects affected only by `GlobalPackageReference`; a nested central manifest -> an explicitly imported ancestor central manifest | [`dotnet-test-plan`](../test-cases/codebase-analysis/dotnet-test-plan), semantic dependency-plan fixtures |
...
| `swift-package` | `SwiftPackageDependency` | `swift` | Swift file -> files in a declared SwiftPM target/product dependency; package source/test -> its manifest; manifest -> its resolved pins; local-package consumers -> the bound local package manifest | [`swift-test-plan`](../test-cases/codebase-analysis/swift-test-plan), semantic Swift dependency-plan fixtures |
...
- Function-scoped dynamic `import()` and `require()` edges are pruned unless the
  ...
  default imports of a class resolve `Service.run()` through that class's static
  member metadata; namespace imports such as `api.Service` still follow the
  exported name. Class
...
- `route-import` deliberately does not apply that function-reachability pruning.
  It remains literal-only, so computed dynamic imports are not graph edges.

...
- Rule suppression must work consistently for every `no-mistakes` rule. Use `no-mistakes` suppression directives, never `guardrails`, and support top-of-file file opt-outs plus inline/next-line opt-outs where line-specific findings exist.

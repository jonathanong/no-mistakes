...
fn detects_use_client() {
    assert_eq!(check("'use client';\nexport default function App() {}"), FileEnvironment::Client);
}

#[test]
fn detects_use_server() {
    assert_eq!(check("'use server';\nexport default function Action() {}"), FileEnvironment::Server);
}

#[test]
fn unknown_environment() {
    assert_eq!(check("export default function App() {}"), FileEnvironment::Unknown);
}

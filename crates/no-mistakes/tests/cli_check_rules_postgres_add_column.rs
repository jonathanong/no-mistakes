...
const RULE: &str = "postgres-no-add-column";

#[test]
fn postgres_no_add_column_fails_for_alter_table() {
    let root = fixture("fail");
    let out = check_fixture_config(&root);
    let body = stdout(&out);
    assert!(!out.status.success(), "expected exit 1: {body}");
    assert!(body.contains(RULE), "{body}");
    assert!(body.contains("ADD COLUMN"), "{body}");
}

#[test]
fn postgres_no_add_column_fails_inside_do_block() {
...
#[test]
fn postgres_no_add_column_passes_create_table() {
...
#[test]
fn postgres_no_add_column_filesystem_runner_discovers_files() {
    let root = fixture("fail");
    ...
    assert!(
        findings
            .iter()
            .any(|finding| finding.rule == no_mistakes::codebase::rules::POSTGRES_NO_ADD_COLUMN),
        "{body}"
    );
}

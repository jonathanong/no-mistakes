...
#[test]
fn malformed_workspace_manifest_does_not_hide_valid_sibling_package() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/analysis-dataset/workspace-malformed-package");
    let files = crate::codebase::ts_source::discover_visible_paths(&root);

    let filesystem_map = load_from_files(&root, &files).unwrap();
    assert_eq!(filesystem_map.packages.len(), 1);
    assert_eq!(filesystem_map.packages[0].name, "@fixture/valid");
    ...
    let cached_map = load_from_source_store(&root, &sources).unwrap();

    assert_eq!(cached_map.packages.len(), 1);
    assert_eq!(cached_map.packages[0].name, "@fixture/valid");
    assert_eq!(sources.json_parse_count(), 3);

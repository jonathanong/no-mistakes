...
use oxc_ast::ast::{CallExpression, ExportDefaultDeclaration, Function, Program};
...
pub(super) fn collect_fused_domain_calls(
    ...
    if !plan.http_calls && !plan.trpc_calls && !plan.call_sites {
...
impl<'a> Visit<'a> for DomainCallVisitor<'a, '_> {
    ...
    fn visit_function(&mut self, function: &Function<'a>, flags: ScopeFlags) {
        ...
        if let Some(name) = name.filter(|_| self.collect_call_sites) {
            self.call_site_scope.push(name);
        }
        walk::walk_function(self, function, flags);
        if name.is_some() && self.collect_call_sites {

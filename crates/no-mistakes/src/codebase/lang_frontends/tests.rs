...
fn fixture(name: &str) -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/lang-frontends").join(name)
}
...
fn python_collects_relative_import_celery_and_django_routes() {
    ...
    let tasks = facts
        ...
        .expect("tasks");
    assert!(tasks.queue_workers.iter().any(|job| job.contains("send_welcome")));
    let enqueue = facts
        .files
        .values()
        .find(|file| file.path.ends_with("enqueue.py"))
        .expect("enqueue");
    assert!(enqueue.queue_enqueues.iter().any(|job| job == "send_welcome"));
    let urls = facts
        .files
        .values()
...
fn php_collects_laravel_route_and_dispatch() {
    ...
    let routes = facts
        ...
        .expect("routes");
    assert!(routes.route_handlers.iter().any(|(route, _)| route == "/api/users"));
    let job = facts
        .files
        .values()
...
fn kafka_extracts_static_topics_and_skips_dynamic() {
    ...
    assert_eq!(topic_identity(Some("orders"), "mail.welcome"), "orders:mail.welcome");
}

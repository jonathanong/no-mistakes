...
#[test]
fn string_computed_static_getter_reads_keep_getter_imports_reachable() {
    let root = crate::codebase::ts_resolver::normalize_path(&fixture("graph-call-narrowing"));
    let tsconfig = TsConfig {
        dir: root.clone(),
        paths: vec![],
    ...
    let graph =
        DepGraph::build_with_plan(&root, &tsconfig, GraphBuildPlan::imports_and_workspace())
            .unwrap();
    let deps = graph.deps_of(
        &[NodeId::file(root.join("src/computed-static-getter-read.mts"))],
        None,
        Some(&[EdgeKind::DynamicImport].into()),
    );
    ...
    assert!(deps.iter().any(|entry| {
        entry.node.as_file()
            == Some(root.join("src/computed-static-getter-loaded.mts").as_path())
    }));
}

#[test]
fn dynamic_computed_static_getter_reads_do_not_keep_getter_imports() {
    let root = crate::codebase::ts_resolver::normalize_path(&fixture("graph-call-narrowing"));
    let tsconfig = TsConfig {
        dir: root.clone(),
        paths: vec![],
    ...
    let graph =
        DepGraph::build_with_plan(&root, &tsconfig, GraphBuildPlan::imports_and_workspace())
            .unwrap();
    let deps = graph.deps_of(
        &[NodeId::file(root.join("src/computed-static-getter-dynamic.mts"))],
        None,
        Some(&[EdgeKind::DynamicImport].into()),
    );
...
#[test]
fn class_alias_static_getter_reads_keep_getter_imports_reachable() {
    let root = crate::codebase::ts_resolver::normalize_path(&fixture("graph-call-narrowing"));
    let tsconfig = TsConfig {
        dir: root.clone(),
        paths: vec![],
    ...
    let graph =
        DepGraph::build_with_plan(&root, &tsconfig, GraphBuildPlan::imports_and_workspace())
            .unwrap();
    let deps = graph.deps_of(
        &[NodeId::file(root.join("src/aliased-static-getter.mts"))],
        None,
        Some(&[EdgeKind::DynamicImport].into()),
    );
    ...
    assert!(deps.iter().any(|entry| {
        entry.node.as_file()
            == Some(root.join("src/aliased-static-getter-loaded.mts").as_path())
    }));
}
...
#[test]
fn multi_hop_dotted_aliases_keep_target_imports_reachable() {
    let root = crate::codebase::ts_resolver::normalize_path(&fixture("graph-call-narrowing"));
    let tsconfig = TsConfig {
        dir: root.clone(),
        paths: vec![],
    ...
    let graph =
        DepGraph::build_with_plan(&root, &tsconfig, GraphBuildPlan::imports_and_workspace())
            .unwrap();
    let deps = graph.deps_of(
        &[NodeId::file(root.join("src/multi-hop-dotted-alias.mts"))],
        None,
        Some(&[EdgeKind::DynamicImport].into()),
    );

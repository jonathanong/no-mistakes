...
#[test]
fn named_imports_preserve_namespace_reexport_identity() {
    let root = crate::codebase::ts_resolver::normalize_path(&fixture("graph-call-narrowing"));
    let tsconfig = TsConfig {
        dir: root.clone(),
        paths: vec![],
    ...
    .unwrap();
    let consumer = root.join("src/namespace-reexport-consumer.mts");
    let target = root.join("src/namespace-reexport-target.mts");
    let calls = graph.deps_of(
        &[NodeId::file(consumer)],
        None,
        Some(&[EdgeKind::Call].into()),

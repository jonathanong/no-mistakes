...
fn swift_references_do_not_select_duplicate_symbols_from_unrelated_packages() {
    ...
    facts
        .files
        .insert(source.clone(), swift_file(&source, Some("App"), vec!["Client"]));
    facts
        .files
        .insert(declared.clone(), swift_file(&declared, Some("Core"), Vec::new()));
    facts
        .files
        .insert(unrelated.clone(), swift_file(&unrelated, Some("Core"), Vec::new()));
    facts.declarations.insert(
        "Client".to_string(),
        BTreeSet::from([declared.clone(), unrelated.clone()]),
    );
    ...
    assert!(edges.contains(&(
        NodeId::file(source),
        NodeId::file(declared),
        EdgeKind::SwiftReference
    )));
    assert!(!edges.iter().any(|(_, target, kind)| {
        *kind == EdgeKind::SwiftReference && target.as_file() == Some(&unrelated)
    }));
}

...
pub(crate) fn lazy_import_deps_of_with_files_facts_and_session(
    request: LazyImportTraversalRequest<'_>,
    session: &crate::codebase::analysis_session::AnalysisSession,
) -> LazyImportTraversalResult {
    let LazyImportTraversalRequest {
        roots,
        root,
        tsconfig,
        max_depth,
    ...
    let (entries, collected) =
        lazy_import_deps_of_with_files_facts_workspace_resolution_cache_and_session(
            LazyImportBuild {
                ...
                graph_files,
                allowed,
                facts: facts.with_source_store(&sources),
                workspace: &workspace,
    ...
    LazyImportTraversalResult {
        entries,
        collected_facts: TsFactMap::from_iter_with_plan(collected, fact_plan),
    }
}

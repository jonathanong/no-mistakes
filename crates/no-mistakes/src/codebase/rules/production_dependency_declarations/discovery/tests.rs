...
#[test]
fn load_workspace_discovers_every_package_json_under_root() {
    ...
    assert!(workspace.package_by_name("@acme/tool").is_some());
}

#[test]
fn load_workspace_dedupes_a_workspace_root_that_matches_the_repo_root() {
    ...
    let workspace = load_workspace(&root, std::slice::from_ref(&root), &files, &sources).unwrap();

    assert_eq!(workspace.packages.len(), 2);
}

#[test]
fn load_workspace_adds_a_distinct_workspace_root_without_duplicating_packages() {
    let root = fixture_root("dev-only-production-import");
    let files = vec![
        root.join("packages/app/package.json"),
        root.join("packages/app/index.mts"),
    ...
    let sources = crate::codebase::rules::source_store_for_files(&files);
    let extra_root = root.join("packages/lib");

    let workspace = load_workspace(&root, &[extra_root], &files, &sources).unwrap();

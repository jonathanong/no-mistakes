...
pub const RULE_ID: &str = "postgres-fk-index";
const DEFAULT_ALLOW_DIRECTIVE: &str = "fk-index-allow";
...
pub(crate) struct Options {
    ...
    pub(crate) allowed_tables: Vec<String>,
}
...
fn compile_options(opts: &Options) -> CompiledOptions {
    CompiledOptions {
        ...
        allowed_columns: opts.allowed_columns.iter().cloned().collect(),
        allowed_tables: opts.allowed_tables.iter().cloned().collect(),
    }
}

use super::{
    condition_values::condition_value, functions::static_case_value, static_bool, InputState,
...
fn condition_values_preserve_known_status_and_short_circuit_operands() {
    let inputs = InputState::new();
    assert_eq!(
        condition_value("success()", &inputs, StaticBool::True),
        Some(StaticValue::Bool(true))
    );
    assert_eq!(
        condition_value("true || github.ref", &inputs, StaticBool::True),
        Some(StaticValue::Bool(true))
    );
    assert_eq!(
        condition_value("false && github.ref", &inputs, StaticBool::True),
...
fn condition_values_remain_unknown_for_unknown_status_and_operands() {
    ...
    assert_eq!(
        condition_value("success()", &inputs, StaticBool::Unknown),
        None
    );
    assert_eq!(
        condition_value("inputs.dynamic || false", &inputs, StaticBool::True),
        None
    );
    assert_eq!(
        condition_value("!github.ref", &inputs, StaticBool::True),
...
fn unary_boolean_values_and_case_predicates_cover_scalar_conversion() {
    let inputs = InputState::from([("dynamic".into(), StaticValue::Unknown)]);
    assert_eq!(
        condition_value("fromJSON('null')", &inputs, StaticBool::True),

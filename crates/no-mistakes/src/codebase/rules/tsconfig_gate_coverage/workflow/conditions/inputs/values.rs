...
use crate::codebase::rules::tsconfig_gate_coverage::workflow::conditions::{
    event_action_value, event_name_value, event_ref_name_value,
    input_value::comparison_literal,
    resolution::{
        github_event_action, github_event_name, github_ref_name, input_name, matrix_name,
        matrix_property_value,
    },
...
pub(super) fn matrix_axis_value(value: &Value) -> Option<StaticValue> {
    match value {
        ...
        // Static object-valued matrix entries are known, truthy values even
        // though GitHub expressions cannot stringify them. Keep that distinct
        // from an omitted matrix property, which coerces to an empty string.
        Value::Mapping(_) => Some(StaticValue::Mapping),

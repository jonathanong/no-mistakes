...
fn container_mapping_shape_valid(container: &Mapping) -> bool {
    only_keys(
        ...
        && string_sequence_valid(container.get("volumes"), CONTAINER_CONTEXTS)
        && container
            .get("options")
...
fn string_sequence_valid(value: Option<&Value>, allowed_contexts: &[&str]) -> bool {
    value.is_none_or(|value| {
        value.as_sequence().is_some_and(|items| {
            items.iter().all(|item| {
                item.as_str()
                    .is_some_and(|value| valid_contextual_value(value, allowed_contexts))
            })
        })
    })
}

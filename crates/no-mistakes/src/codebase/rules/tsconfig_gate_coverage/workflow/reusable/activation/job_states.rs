...
use crate::codebase::rules::tsconfig_gate_coverage::workflow::conditions::{
    inputs_with_matrix_values, inputs_with_needs_results,
    inputs_with_static_strategy_position_values, inputs_with_strategy_configuration_values,
    InputState, MatrixState, StaticValue,
};
use crate::codebase::rules::tsconfig_gate_coverage::workflow::reusable::validation::{
    static_matrix_combinations_for_inputs, strategy_context_values_for_inputs, MatrixCombinations,
};

pub(super) struct JobStates {
    matrix_inputs: BTreeMap<String, Vec<InputState>>,
    matrix_job_totals: BTreeMap<String, Option<usize>>,
...
impl JobStates {
    pub(super) fn new(jobs: &serde_yaml::Mapping, inputs: &InputState) -> Option<Self> {
        ...
        let mut matrix_job_totals = BTreeMap::new();
        ...
        for (job_id, job) in jobs {
            ...
            let (inputs, job_total) = match combinations {
                MatrixCombinations::Static(combinations) => {
                    let job_total = combinations.len();
                    (
                        combinations
                            .iter()
                            .enumerate()
                            .map(|(index, values)| {
                                ...
                                inputs_with_static_strategy_position_values(
                                    &inputs, job, index, job_total,
                                )
                            })
                            .collect(),
                        Some(job_total),
                ...
                MatrixCombinations::Dynamic(_) => (
                    vec![inputs_with_matrix_values(
                        inputs,
                        &BTreeMap::new(),
                        MatrixState::Dynamic,
                    )],
            ...
            matrix_job_totals.insert(job_id.clone(), job_total);
        ...
        Some(
            self.inputs_for(job_id)?
                ...
                .map(|inputs| {
                    ...
                    let (fail_fast, max_parallel) = strategy_context_values_for_inputs(
                        job,
                        &inputs,
                        *self
                            .matrix_job_totals
                            .get(job_id)
                            .expect("matrix totals and inputs share normalized job keys"),
                    );
                    inputs_with_strategy_configuration_values(&inputs, job, fail_fast, max_parallel)
                })
                .collect(),
        )
    }

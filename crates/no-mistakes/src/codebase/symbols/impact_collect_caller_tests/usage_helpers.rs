...
fn dynamic_usage_helpers_ignore_non_module_and_malformed_bindings() {
    assert!(
        dynamic_module_bindings("const utils = await import('./utils.mts');").contains("utils")
    );
    assert!(
        dynamic_module_bindings("const readDate = require('./utils.mts').parseDate;").is_empty()
    );
    assert!(destructured_symbol_aliases(
        "const { parseDate = await import('./utils.mts');",
        "parseDate"
    )
    .is_empty());
    ...
    assert!(dynamic_symbol_aliases_in_source(

...
fn prepare_local_caller_context_resolves_workspace_packages_once() {
    ...
    let remapper =
        crate::codebase::ts_source::FrozenPathRemapper::from_paths(visible_files.iter().cloned());
...
fn prepare_local_caller_context_never_rederives_its_supplied_inputs() {
    ...
    let remapper =
        crate::codebase::ts_source::FrozenPathRemapper::from_paths(visible_files.iter().cloned());
    let context =
        prepare_local_caller_context(&empty_facts, &empty_workspace, &visible_files, &remapper);

...
#[test]
fn signature_impact_treats_chained_local_barrels_as_exports() {
    let out = run_capture(impact_args("parseDate", Format::Json));
    let v: serde_json::Value = serde_json::from_str(&out).unwrap();

    assert!(v["exports"].as_array().unwrap().iter().any(|entry| {
        entry["file"] == "chained-local-date-barrel.mts" && entry["symbol"] == "publicParse"
    }));
    assert!(!v["productionCallers"].as_array().unwrap().iter().any(|entry| {
        entry["file"] == "chained-local-date-barrel.mts" && entry["symbol"] == "publicParse"
    }));
    assert!(v["productionCallers"].as_array().unwrap().iter().any(|entry| {
        entry["file"] == "chained-local-barrel-consumer.mts"
            && entry["symbol"] == "parseChainedDate"
    }));
}

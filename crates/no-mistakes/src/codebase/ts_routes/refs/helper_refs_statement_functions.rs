fn collect_helper_refs_from_function_body<'a>(
    func: &'a oxc_ast::ast::Function<'a>,
    source: &str,
    file: &str,
    router_bindings: &mut RouterBindings<'a>,
    helper_bindings: &mut RouteHelperBindings,
    local_helpers: &HashSet<String>,
    refs: &mut Vec<RouteHelperRef>,
) {
    let Some(body) = &func.body else {
        return;
    };
    let mut scoped_bindings = router_bindings.clone();
    let mut scoped_helper_bindings = helper_bindings.clone();
    remove_shadowed_function_binding(func, &mut scoped_bindings);
    remove_shadowed_parameters(&func.params, &mut scoped_bindings);
    remove_shadowed_helper_function_binding(func, &mut scoped_helper_bindings, local_helpers);
    remove_shadowed_helper_parameters(&func.params, &mut scoped_helper_bindings);
    collect_router_bindings_for_scope(&body.statements, &mut scoped_bindings);
    collect_scope_helper_bindings(&body.statements, &mut scoped_helper_bindings, local_helpers);
    for stmt in &body.statements {
        collect_helper_refs_from_statement(
            stmt,
            source,
            file,
            &mut scoped_bindings,
            &mut scoped_helper_bindings,
            local_helpers,
            refs,
        );
    }
}

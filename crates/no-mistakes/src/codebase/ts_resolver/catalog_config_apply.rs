    ...
    fn apply_own(
        ...
        if let Some(compiler) = compiler {
            ...
            if let Some(base_url) = compiler.get("baseUrl") {
                self.base_url = Some(config_relative_path(
                    base_url,
                    dir,
            ...
            if let Some(allow_js) = compiler.get("allowJs") {
                let Some(parsed) = allow_js.as_bool() else {
                    return Err(format!(
                        "{} compilerOptions.allowJs must be a boolean",
                        path.display()
                    ));
                };
                self.allow_js = Some(parsed);
            }
            if let Some(out_dir) = compiler.get("outDir") {
                self.out_dir = Some(config_relative_path(
                    out_dir,
                    dir,
            ...
            if let Some(module_resolution) = compiler.get("moduleResolution") {
                let Some(parsed) = module_resolution.as_str() else {
                    return Err(format!(
        ...
        if let Some(references) = value.get("references") {
            self.references = reference_values(references, path)?
                .into_iter()

...
/// `CREATE TABLE` statements parse. Column lists on referential
/// `SET NULL` / `SET DEFAULT` (`ON DELETE SET NULL (column)`) are removed
/// before parsing; the action keyword and constraint name stay.

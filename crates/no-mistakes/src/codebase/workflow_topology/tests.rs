...
//! Fixture-backed legacy-field parity tests against the retired TypeScript
//! engine, an enriched schema-v1 golden, and targeted unit tests for the
//! trickiest pure logic.
//!
//! Legacy golden files (`expected*.json`) under `test-cases/workflow-topology/`
...
fn mermaid_matches_ts_engine_for_concurrency_locks() {
    ...
    assert_eq!(render_mermaid(&root), golden(&root, "expected.mmd"));
    schema_compat::assert_legacy_golden(&render(&root, &[]), &golden(&root, "expected.json"));
}
...
macro_rules! golden_test {
    ($name:ident, $fixture:expr) => {
        ...
        fn $name() {
            ...
            schema_compat::assert_legacy_golden(
                &render(&root, &[]),
...
fn invalid_fixture_matches_ts_engine_except_malformed_message_text() {
    ...
    let actual =
        strip_malformed_messages(schema_compat::legacy_projection(json(&render(&root, &[]))));

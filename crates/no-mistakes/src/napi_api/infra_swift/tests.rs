use super::*;
use crate::napi_api::options::parse_options;
use serde_json::json;
...
fn infra_resource_refs_impl_returns_referencing_blocks() {
    ...
    .to_string();
    let output =
        infra_resource_refs_json_impl(crate::napi_api::options::test_json_arg(options)).unwrap();
...
fn infra_test_for_impl_returns_covering_tests() {
    ...
    .to_string();
    let output =
        infra_test_for_json_impl(crate::napi_api::options::test_json_arg(options)).unwrap();
...
fn infra_impls_require_their_arguments() {
    ...
    assert!(
        infra_resource_refs_json_impl(crate::napi_api::options::test_json_arg(options.clone()))
            .is_err()
    );
    assert!(
        infra_outputs_json_impl(crate::napi_api::options::test_json_arg(options.clone())).is_err()
    );
...
fn swift_importers_impl_returns_importers() {
    ...
    .to_string();
    let output =
        swift_importers_json_impl(crate::napi_api::options::test_json_arg(options)).unwrap();
...
fn swift_impls_require_file() {
    ...
    assert!(
        swift_importers_json_impl(crate::napi_api::options::test_json_arg(options.clone()))
            .is_err()
    );
    assert!(
        swift_test_targets_json_impl(crate::napi_api::options::test_json_arg(options)).is_err()
    );
}

...
fn serializes_the_public_validation_result() {
    ...
    let output = validate_mermaid_markdown_json_impl(crate::napi_api::options::test_json_arg(options)).unwrap();
...
fn rejects_missing_content_and_unknown_options() {
    let missing_content = validate_mermaid_markdown_json_impl(crate::napi_api::options::test_json_arg(json!({}).to_string())).unwrap_err();
    assert!(missing_content
        .to_string()
        .contains("missing field `content`"));

    let unknown = validate_mermaid_markdown_json_impl(
        crate::napi_api::options::test_json_arg(json!({ "content": fixture("valid.md"), "unknown": true }).to_string(),)
    )
    .unwrap_err();

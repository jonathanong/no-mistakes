    ...
    let standalone = [
        ...
        parse_json(
            crate::napi_api::playwright_check_json_impl(
                crate::napi_api::options::test_json_arg(json!({ "root": root, "assertUniqueHtmlIds": true }).to_string(),)
            )
            .unwrap(),
        ),
        parse_json(
    ...
    let standalone = [
        parse_json(
            crate::napi_api::react_analyze_json_impl(
                crate::napi_api::options::test_json_arg(json!({ "root": root, "targets": targets }).to_string(),)
            )
            .unwrap(),
        ),
        parse_json(
...
fn prepared_server_mixed_views_match_standalone() {
    ...
    let standalone = [
        ...
        parse_json(
            crate::napi_api::server_contracts_json_impl(crate::napi_api::options::test_json_arg(json!({ "root": root }).to_string()))
                .unwrap(),
        ),
    ];

...
fn baseline_and_audit(name: &str) -> (serde_json::Value, serde_json::Value) {
    ...
    let baseline: serde_json::Value =
        serde_json::from_str(&check_json_impl(crate::napi_api::options::test_json_arg(json!({ "root": root }).to_string())).unwrap())
            .unwrap();
    let audit: serde_json::Value = serde_json::from_str(
        &check_json_impl(crate::napi_api::options::test_json_arg(json!({ "root": root, "includeSuppressed": true }).to_string())).unwrap(),
    )
    .unwrap();
...
fn check_json_reports_tracked_artifacts_below_source_skip_directories() {
    ...
    let output = check_json_impl(crate::napi_api::options::test_json_arg(json!({ "root": fixture.path() }).to_string())).unwrap();
...
fn check_json_optionally_accounts_for_suppressed_ordinary_rule_findings() {
    ...
    let baseline = check_json_impl(crate::napi_api::options::test_json_arg(json!({ "root": root }).to_string())).unwrap();
...
fn ordinary_check_keeps_later_react_component_after_earlier_component_is_suppressed() {
    ...
    let output = check_json_impl(crate::napi_api::options::test_json_arg(json!({ "root": root }).to_string())).unwrap();
    let value: serde_json::Value = serde_json::from_str(&output).unwrap();
...
fn check_json_reports_both_markdown_rule_ids() {
    ...
    let output = check_json_impl(
        crate::napi_api::options::test_json_arg(json!({ "root": fixture.path(), "config": ".no-mistakes.yml" }).to_string(),)
    )
    .unwrap();
    let value: serde_json::Value = serde_json::from_str(&output).unwrap();

...
#[test]
fn check_json_audits_reachable_findings_from_disabled_tests() {
    let (baseline, audit) =
...
fn check_json_rejects_disabled_malformed_helpers_reached_from_active_integration_tests() {
    ...
    let error = check_json_impl(crate::napi_api::options::test_json_arg(json!({ "root": root }).to_string())).unwrap_err();

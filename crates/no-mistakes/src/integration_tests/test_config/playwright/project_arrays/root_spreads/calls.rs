...
pub(super) fn call_project_options(
    ...
    let result = if let Some(expression) = ctx.bindings.get(name).copied() {
        ...
        imported_project_options(&import, ctx.path, ctx)
    } else {
        Ok(None)
    };
...
fn body_return_project_options(
    ...
    if !body_bindings.is_empty() {
        ...
        return body_return_project_options_without_locals(body, &mut scoped);
    }
    body_return_project_options_without_locals(body, ctx)
}

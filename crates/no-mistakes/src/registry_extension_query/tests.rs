...
#[test]
fn prepared_run_reuses_one_source_read_and_parse_attempt() {
    let observer = crate::diagnostics::InvocationObserver::new(true);
    let session = crate::codebase::analysis_session::AnalysisSession::new(Some(observer));
    let file = fixture().join("register-call.ts");

    let first = run_with_session(&session, &fixture(), Path::new("register-call.ts")).unwrap();
    let second = run_with_session(&session, &fixture(), Path::new("register-call.ts")).unwrap();

...
#[test]
fn prepared_request_keeps_dotnet_semantic_records_and_filters_only_handled_paths() {
    ...
    assert_eq!(
        prepared.dotnet_dependency_analysis.artifacts[0].changed_dependencies,
        std::collections::BTreeSet::from(["Beta".to_string()])
    );

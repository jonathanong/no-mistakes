...
fn closing_fence_rejects_excess_indent_and_trailing_content() {
    ...
    assert!(!is_closing_fence_line(b"```\x0c", b'`', 3, 0, 0));
    assert!(!is_closing_fence_line(b"```\x0b", b'`', 3, 0, 0));
    assert!(is_closing_fence_line(b"``` \t", b'`', 3, 0, 0));
}
...
fn blockquote_and_line_end_helpers_cover_boundary_forms() {
    ...
    assert_eq!(strip_blockquote_prefix(b">quote"), (&b"quote"[..], 1, 1));
    assert_eq!(
        strip_blockquote_prefix(b" > > quote"),
        (&b"quote"[..], 2, 5)
    );
    ...
    assert_eq!(markdown_column_after(0, b"-\t"), 4);
    assert_eq!(markdown_column_after(0, b"- \t"), 4);
    assert_eq!(markdown_column_after(1, b"\t"), 4);
    assert!(is_closing_fence_suffix(b""));
    assert!(is_closing_fence_suffix(b" \t"));
    assert!(!is_closing_fence_suffix(b"\x0c"));
    assert!(!is_closing_fence_suffix(b"\x0b"));
}

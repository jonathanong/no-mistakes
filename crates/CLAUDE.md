...
`git ls-files -z -t --stage --cached --others --deleted --exclude-standard` process —
index mode classifies tracked regular files without `lstat`. Thread that
snapshot through any path that would rediscover files, e.g. the

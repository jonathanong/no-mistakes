...
use crate::report::types::{CacheKind, FetchOccurrence, FetchSide};
use oxc_ast::ast::{Argument, CallExpression, Expression};
use oxc_span::GetSpan;

/// Try to extract a FetchOccurrence from a `fetch(...)` call expression.
/// Returns None if the call is not a plain (non-shadowed) `fetch` call.
pub(crate) fn try_extract_fetch<'a>(
...
/// Enter a cache wrapper: save previous cached state, set new state.
/// Returns (previous_cached_function, previous_cached_kind).
pub(crate) fn enter_cache_wrapper<'a>(
    expr: &CallExpression<'a>,
...
/// Restore cached state after leaving a cache wrapper.
pub(crate) fn leave_cache_wrapper(

...
pub(crate) struct FetchVisitor<'a> {
    pub(crate) source: &'a str,
    pub(crate) file: String,
    pub(crate) fetches: Vec<crate::report::types::FetchOccurrence>,
    pub(crate) is_client: bool,
    pub(crate) is_route_handler: bool,
    pub(crate) cached_function: Option<String>,
    pub(crate) cached_kind: Option<CacheKind>,
    pub(crate) fetch_scope_stack: Vec<FetchScope>,
    pub(crate) in_var_declaration: bool,
}

#[derive(Default)]
pub(crate) struct FetchScope {
    pub(crate) shadowed_identifiers: HashSet<String>,
    pub(crate) tracks_var_bindings: bool,
}
...
impl<'a> FetchVisitor<'a> {
    ...
    pub(crate) fn enter_fetch_scope(&mut self, tracks_var_bindings: bool) {
    ...
    pub(crate) fn leave_fetch_scope(&mut self) {
        self.fetch_scope_stack.pop();
    }

    pub(crate) fn mark_fetch_shadowed(&mut self) {
        if let Some(scope) = self.fetch_scope_stack.last_mut() {
            scope.shadowed_identifiers.insert("fetch".to_string());
        }
    }

    pub(crate) fn mark_identifier_shadowed_in_var_scope(&mut self, name: &str) {
    ...
    pub(crate) fn mark_identifier_shadowed(&mut self, name: &str) {
        if let Some(scope) = self.fetch_scope_stack.last_mut() {
            scope.shadowed_identifiers.insert(name.to_string());
        }
    }

    pub(crate) fn is_fetch_shadowed(&self) -> bool {

...
Configured `tests.python|go|rust|rails|php|java` and `tests.dotnet.projects` /
`tests.swift.packages` participate in the canonical graph. Query them with
`dependents --relationship python|go|rust|ruby|php|java|dotnet|swift` and
`tests plan python|go|cargo|rails|php|java|dotnet|swift`.
...
- Python / Go / Cargo / Rails / PHP / Java: `no-mistakes tests plan python|go|cargo|rails|php|java --changed-file <file> --format paths`
...
| Which Python/Go/Rust/Ruby/PHP/Java/Swift/.NET files depend on this file? | `no-mistakes dependents <file> --relationship python\|go\|rust\|ruby\|php\|java\|swift\|dotnet --format json` |
...
| Which Python/Go/Cargo/Rails/PHP/Java tests should rerun? | `no-mistakes tests plan python\|go\|cargo\|rails\|php\|java --changed-file <file> --format paths` |
...
- `--test vitest|playwright|cargo|dotnet|swift|python|go|rails|php|java|jest` to filter to test files.
- `--relationship import|import-static|import-dynamic|import-type|import-require|route-import|workspace|package|test|route|queue|trpc|resource|md|ci|workflow|workflow-job|workflow-step|workflow-needs|workflow-uses|workflow-run|workflow-artifact|http|process|asset|react|dotnet|swift|terraform|python|go|rust|ruby|php|java|all`.

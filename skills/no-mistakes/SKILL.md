---
name: no-mistakes
description: "Core: TS/JS module graph (imports, dependents, exports, test impact, Playwright, React, queue/server, fetches, lockfile, no-mistakes checks). Also: CI, Terraform/OpenTofu, Swift/.NET, and configured Python/Go/Rust/Rails/PHP/Java graphs plus tests plan. Prefer over rg when a question spans >2 workspace dirs or >5 import hops."
...
Configured `tests.python|go|rust|rails|php|java|kotlin` and `tests.dotnet.projects` /
`tests.swift.packages` participate in the canonical graph. Query them with
`dependents --relationship python|go|rust|ruby|php|java|kotlin|dotnet|swift` and
`tests plan python|go|cargo|rails|php|java|kotlin|dotnet|swift`.
...
`--relationship asset`. Go/Python/Rust/Rails/PHP/Java/.NET graphs require explicit `tests.<lang>` config — see https://github.com/jonathanong/no-mistakes/blob/main/docs/feature-parity.md.
...
- Python / Go / Cargo / Rails / PHP / Java / Kotlin: `no-mistakes tests plan python|go|cargo|rails|php|java|kotlin --changed-file <file> --format paths`
...
| Which Python/Go/Rust/Ruby/PHP/Java/Kotlin/Swift/.NET files depend on this file? | `no-mistakes dependents <file> --relationship python\|go\|rust\|ruby\|php\|java\|kotlin\|swift\|dotnet --format json` |
...
| Which Python/Go/Cargo/Rails/PHP/Java/Kotlin tests should rerun? | `no-mistakes tests plan python\|go\|cargo\|rails\|php\|java\|kotlin --changed-file <file> --format paths` |
...
- `--test vitest|playwright|cargo|dotnet|swift|python|go|rails|php|java|kotlin|jest` to filter to test files.
- `--relationship import|import-static|import-dynamic|import-type|import-require|route-import|workspace|package|test|route|queue|trpc|resource|md|ci|workflow|workflow-job|workflow-step|workflow-needs|workflow-uses|workflow-run|workflow-artifact|http|process|asset|react|dotnet|swift|terraform|python|go|rust|ruby|php|java|kotlin|all`.

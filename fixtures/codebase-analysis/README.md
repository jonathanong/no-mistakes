...
| `codebase-intel/` | `tests/cli_codebase_acceptance.rs`; `src/dependencies/graph/tests.rs` — graph edge kinds and virtual nodes |
| `cross-boundary-monorepo/` | `tests/cli_codebase_acceptance.rs` — workspace, tsconfig, barrel, symbol, and baseUrl traversal |
| `dependents-basic/` | `tests/cli_codebase_acceptance.rs` — fixture-only `dependents` traversal, output, and relationship coverage |
| `workspace-conditional-exports/` | `src/workspaces/tests.rs::fixture_workspace_conditional_exports` |
...
| `routes-backend/` | `src/ts_routes/defs_backend/tests.rs::fixture_routes_backend_extracts_nested_router_pattern` |
| `simple/` | `src/dependencies/graph/tests.rs`; `tests/cli_dependencies.rs`; `tests/cli_guardrails.rs` |
| `symbols-output/` | `src/symbols/tests.rs` — fixture-backed symbols output and formatting coverage |

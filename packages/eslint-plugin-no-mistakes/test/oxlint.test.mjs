...
`,
  ...
  it("rejects definitely undefined required options through the plugin", () => {
    const root = mkdtempSync(join(tmpdir(), "pac-oxlint-"));
    try {
      copyFileSync(
        resolve(
          __dirname,
          "../../../test-cases/eslint-plugin/require-options-on-imported-call/fixture/undefined-missing.ts",
        ),
        join(root, "fixture.ts"),
      );
      writeFileSync(

...
test("installer main downloads into the direct bin target", async () => {
  ...
  await main(async (...args) => {
    calls.push(args);
    return "/tmp/server-ast-routes";
  });
  assert.equal(calls.length, 1);
  assert.deepEqual(calls[0].slice(0, 2), ["server-ast-routes", "jonathanong/no-mistakes"]);
  assert.equal(calls[0][2].vendorDir, join(PACKAGE_ROOT, "bin"));
  assert.equal(calls[0][2].destinationName, "server-ast-routes");
});
